...
def kl_divergence(
    probs_p: torch.Tensor, probs_q: torch.Tensor
) -> torch.Tensor:
    """
...
def layer_evolution_uq(
    ...
    with torch.no_grad():
        ...
        for layer_states_idx in range(
            layer_amount - layers_from_end, layer_amount - 1
        ):
            ...
            if "kl_divergence" in metric_name:
                layer_uq = kl_divergence(
                    layer_distribution, last_layer_distribution
                )
